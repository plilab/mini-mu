...
graphMain = do
  let dotGraph = graphElemsToDot nonClusteredParams nodes edges
      ...
      nodes = [(1, [A.Label $ A.StrLabel $ pack "Node 1"]),
          ...
          let nextConfigs = step current
              ...
              (graph', nodeCount', visited'') = 
                foldl (\(g, nc, vs) next -> 
                      ...
                      ) (graph, nodeCount, visited') nextConfigs
          in (graph', nodeCount')
...
evalGraphParams = nonClusteredParams { globalAttributes = [GraphAttrs [A.RankDir A.FromLeft]]
